...
class AwaitingStartViewModel(private val savedStateHandle: SavedStateHandle) : ViewModel() {
    ...
    fun pollExperiment(navController: NavController) {
        ...
        viewModelScope.launch {
            ...
            while (isAwaiting) {
                ...
                if (joinedExperiment?.stage == AssessmentStage.WAITING.stage) {
                    Log.d("EXPPOLL", "Awaiting Change to Status")
                } else {
                    isAwaiting = false
                    Log.d("EXPPOLL", "Status Changed to " + joinedExperiment?.stage)
                    navController.navigate(Screen.Calibration.route.replace(
